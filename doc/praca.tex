...
\subsection{Logika kombinacyjna}
...
\subsubsection{Licznik}
Zastosowanie licznika powtarza się najczęściej.
...
Przyciski na płytce są mechaniczne. Pojedyncze wciśnięcie jest obarczone początkowymi drganiami styków, co dla FPGA jest wielokrotnym wciśnięciem tego samego przycisku w bardzo krótkich odstępach czasu. Niepożądanego efektu można się pozbyć przez $Debouncer$-a bazującego na liczniku.
\lstinputlisting[label=Debouncer,caption=Debouncer.v]{../projects/generic/Debouncer.v}

\subsubsection{Spowalniacz zegara}

\lstinputlisting[label=Odwracacz,caption=Bits\_Reverse.v]{../projects/generic/Bits\_Reverse.v}
...
Wykrywanie zbocza zrealizowane jest przy użyciu rejetru przesuwnego zapisującego stale dwie ostatnie wartości śledzone sygnału w takt podanego zegara. Flagi $pos$ i $neg$ sygnalizują zbocze przy wykryciu właściwego wzorca.
\lstinputlisting[label=Zbocza,caption=Edge\_Detector.v]{../projects/generic/Edge\_Detector.v}
...
Odwrócenie kolejności bitów w rejestrze wymaga wygenerowania kodu. Niedogodność języka została schowana w dedykowanym module.
\lstinputlisting[label=Odwracacz,caption=Bits\_Reverse.v]{../projects/generic/Bits\_Reverse.v}
