...
\begin{minipage}{11cm}
   ...
   %% \caption{Demostracja logiki alternatywy zrealizowanej na połączeniach}
\end{minipage}
\end{table}
...
Prawa te są niezbędne do zaprojektowania sieci lub jej optymalizacji.

\subsection{Bramki logiczne}
Są to układy scalone realizujące wyspecyfikowane przez producenta operacje logiczne. Na samych przedstawionych połączeniach można osiągnąć logikę koniunkcji lub alternatywy. Nie da się jednak sygnałowi zaprzeczyć. Możnaby problem obejść używając przekaźniki lub tranzystory, tudzież ich zestawy odpowiednio połączone i zatopione w gotowych scalakach logicznych.

Podane są oznaczenia schematyczne operacji.



Dokumentacja układu określa położenia pinów wejściowych i wyjściowych każdej z dostępnych operacji. Przykładowe kostki polskiej PRL-owskiej produkcji firmy CEMI.

% TODO nie mozliwe przeczenia, xor na samych drutach

Zadanie nowego stanu wejścia wymaga czasu na przełączenie się wewnętrznych tranzystorów zanim ustali się właściwy wynik na wyjściu. Opóźnienie to nazywane jest czasem propagacji bramki.
