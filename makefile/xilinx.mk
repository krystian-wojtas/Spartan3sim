...
# path to this generic makefile from any project
generic_path = ../../generic/makefile

libmks = $(patsubst %,$(libdir)/%/module.mk,$(libs))
mkfiles = Makefile $(libmks) $(generic_path)/xilinx.mk
include $(libmks)
...
.PHONY: default xilinx_cores clean distclean twr etwr
...
# orginal target contains extra bmm file dependency with block ram memmory map description, I don't need it
# to add it ngdbuild ... -bm $(project).bmm
$(project).ngd: $(project).ngc ../$(project).ucf
	$(xil_env); ngdbuild $(intstyle) -dd _ngo -nt timestamp -uc ../$(project).ucf -p $(part) "Top.ngc" Top.ngd
junk += $(project).ngd $(project).bld

$(project).ngc: $(vfiles) $(local_corengcs) $(project).scr $(project).prj
	$(xil_env); xst $(intstyle) -ifn Top.xst -ofn Top.scr
...
$(project).prj: $(vfiles) $(mkfiles)
	...
	sort -u $(project).tmpprj > Top.prj
	rm -f $(project).tmpprj
junk += $(project).prj

optfile += $(wildcard $(project).opt)
top_module ?= $(project)
$(project).scr: $(optfile) $(mkfiles) $(generic_path)/xilinx.opt
	echo "run" > $@
	echo "-p $(part)" >> $@
	echo "-top $(top_module)" >> $@
	echo "-ifn Top.prj" >> $@
	echo "-ofn $(project).ngc" >> $@
	cat $(generic_path)/xilinx.opt $(optfile) >> $@
junk += $(project).scr
...
configure:
	$(xil_env); impact -batch <<< "` cat ../../generic/makefile/impact_batch.tpl | sed "s:BITSTREAM_FILE:$$PWD/$(project).bit:" `"

compile_sim:
#	echo "/home/c/Dokumenty/projekty/jezyki opisu sprzetu/mgr/Spartan3Sim/projects/rs232/project/TopTest_beh.prj"
#	echo "$$PWD/TopTest_isim_beh.prj"
#	$(xil_env); fuse $(intstyle) -incremental -lib unisims_ver -lib unimacro_ver -lib xilinxcorelib_ver -o "$$PWD/TopTest_isim_beh.exe" -prj "/home/c/Dokumenty/projekty/jezyki opisu sprzetu/mgr/Spartan3Sim/projects/rs232/project/TopTest_beh.prj" work.TopTest work.glbl
	$(xil_env); fuse $(intstyle) -incremental -lib unisims_ver -lib unimacro_ver -lib xilinxcorelib_ver -o "$$PWD/TopTest_isim_beh.exe" -prj "$$PWD/TopTest_beh.prj" work.TopTest work.glbl



sim:
	"$$PWD/TopTest_isim_beh.exe" $(intstyle) -gui -tclbatch isim.cmd  -wdb "$$PWD/TopTest_isim_beh.wdb"